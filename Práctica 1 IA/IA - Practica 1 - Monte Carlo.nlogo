...
to jugar
  ...
  if(turno = 1 and jugado?)[
    print "Que calaja tan bonitaa :D"
    let m MCTS:UCT (list matriz-global 0) 4
    print(word "Al final, he decidido" m)
    set turno ((turno + 1) mod 2)
...
to-report MCTS:get-rules[s]
  ...
  ifelse p = 0 [
    report filter [x -> item x ls1 != 0](range 7)
  ][
    report filter [x -> item x ls2 != 0](range 7)
  ]
